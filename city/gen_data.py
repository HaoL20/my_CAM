...
class City_Dataset(data.Dataset):
    def __init__(self, data_root, size, num_classes=19, ignore_label=-1, resize=True):
        ...
        # 获取数据集路径列表
        self.items = [i for i in open('train.txt')]         # 2975张训练集  

    def __getitem__(self, index):
        id_image, id_label = self.items[index].strip('\n').split(' ')
        image_path = os.path.join(self.data_root, 'leftImg8bit') + id_image
        label_path = os.path.join(self.data_root, 'gtFine') + id_label
...
def main():
    data_root = 'F:\dataset\city_1024_512'
    image_path = 'images'
    label_path = 'labels'
    ...
    for i, (images, labels, image_name, label_name) in tqdm(enumerate(data_loader)):
        ...
        for idx_area, area in enumerate(areas):                             # 遍历全部左上角起点位置
            ...
            if cur_label and is_save:
                ...
                output_col.save('{}/random_{}_{}_{}_class_{}.png'.format(label_path, random_crop, label_name[0], idx_area, ''.join(cur_label)))
